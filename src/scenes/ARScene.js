...
export default class ARScene extends React.Component {
  ...
  getNearbyPlaces = async () => {
    this.setState({
      nearbyPlaces: require('../beacons.json'),
    });
        ...
        .then((responseJson) => {
          ...
          if (responseJson.status === 'OK') {
            const places = responseJson.results.map((rawPlace) => {
              console.log('name: ' + rawPlace.name);
              console.log('id: ' + rawPlace.place_id); //id
              console.log('lat: ' + rawPlace.geometry.location.lat);
              console.log('lng: ' + rawPlace.geometry.location.lng);
              return {
  ...
  placeARObjects = () => {
    ...
    const ARTags = this.state.nearbyPlaces.map((item) => {
      ...
      /*scale={[this.state.scale, this.state.scale, this.state.scale]}
          ...
          <Viro3DObject
              key={item.id}
              source={require('./res/models/OBJ/monkey.obj')}
              resources={[
                require('./res/models/OBJ/monkey.mtl'),
                require('./res/models/OBJ/monkey_texture.jpeg'),
              ]}
              position={[coords.x, 0, coords.z]}
              scale={[1, 1, 1]}
              type="OBJ"
